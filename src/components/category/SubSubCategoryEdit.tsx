...
interface SubSubCategoryEditProps {
  isOpen: boolean;
...
export default function SubSubCategoryEdit({ isOpen, onClose, previous, refresh }: SubSubCategoryEditProps) {
  const api = useApi();
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    setLoading(true);
    api
      .editSubSubCategory(formData, previous?.id)
      .then(() => {
        setLoading(false);
        onClose();
        refresh()
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Sub Sub Category Edit">
